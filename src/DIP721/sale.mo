...
shared(msg) actor class NFTSale(
    ...
    private var mapOfListings = HashMap.HashMap<ListID, Listing>(1, Nat.equal, Hash.hash);
    private stable var txs: [TxRecord] = [];
    private stable var txIndex: Nat = 0;

    public query func getListedNFTs() : async [(ListID,Listing)] {
      let ids = Iter.toArray(mapOfListings.entries());
      return ids;
    };
    ...
    /**
      * _isApprovedForAll is to return if the owner and operator had been approved to transfer/transferFrom the NFT token
      * @owner Owner of the NFT token
      * @operator Principal ID to allow contract to operate or transfer the NFT
      * @return Bool when found had been approved
      */
    ...
    /**
      * _addTokenTo is to add in new token into users TrieSet array
      * @to Principal ID to be added into
      * @tokenId Token ID to be added into the Principal ID
      */
    ...
    /**
      * _removeTokenFrom is to remove NFT token from existing owers users TrieSet array
      * @owner Current owner of the NFT Token
    ...
    /**
      * _clearApproval is to remove approval after transaction occur for transfer/transferFrom
      * @owner Principal ID to be check the approval
      * @tokenId NFT Token ID
      */
    ...
    /**
      * _transfer is internal transfer function
      * @to Principal ID to transfer the NFT to
      * @tokenId NFT Token ID
      */
    private func _transfer(to: Principal, tokenId: Nat) {
        assert(_exists(tokenId));
        assert(_isListed(tokenId));

        switch(tokens.get(tokenId)) {
            case (?info) {
                _removeTokenFrom(info.owner, tokenId);
                _addTokenTo(to, tokenId);
                _changeMapListing(to,tokenId);
                info.owner := to;
                tokens.put(tokenId, info);
            };
            case (_) {
    ...
    private func _changeMapListing(newOwner:Principal,id:Nat){
        switch(mapOfListings.get(id)){
            case(?list){
                if(not list.isListed){
                    if(list.creator != newOwner){
    ...
    private func _isListed(tokenId:Nat) : Bool{
        switch(mapOfListings.get(tokenId)){
            case (?list){
                if(not list.isListed){
                    return true;
                }else{
    ...
    public shared(msg) func buy(amount: Nat,listedId:Nat): async Result.Result<Nat, Text> {
        ...
        //     case(_) {};
        // };
        // let nftPrice: Nat = await getListingPrice(listedId);
        // let tokenActor: TokenActor = actor(Principal.toText(info.paymentToken));
        // let resultTransfer : Bool = await tokenActor.transferFrom(msg.caller, Principal.fromActor(this), amount * nftPrice);
        ...
        // switch(resultTransfer) {
            // case(true) {
                ...
                // saleInfo := ?info;
                // mapOfListings.delete(listedId);
                switch(tokens.get(listedId)){
                    case null {return #err(("Failed"))};
                    case (?info){
                        if(info.owner != msg.caller){
                            info.owner := msg.caller;
                            tokens.put(listedId,info);
                        }
                    };
                };
                switch (mapOfListings.get(listedId)) {
                    case (null) {return #err(("Failed"))};
                    case (?result) {
                      if(result.creator != msg.caller){
                        let newListing : Listing = {
                        ...
                        mapOfListings.put(listedId,newListing);
                        return #ok((amount));
                      }else{
                        return #err(("failed"));
                      }
    ...
    public shared(msg) func mint(to: Principal, metadata: ?TokenMetadata): async MintResult {
        ...
        mapOfListings.put(next,listingData);
        tokens.put(totalSupply_, token);
        _addTokenTo(to, totalSupply_);
        totalSupply_ += 1;
        next += 1;
    ...
    public shared(msg) func listNFT(tokenid:Nat,price:Nat):async Result.Result<(Bool),(Bool)>{
      switch (mapOfListings.get(tokenid)) {
        case (null) {return #err((false))};
        case (?result) {
            if(result.creator == msg.caller){
                let newListing : Listing = {
                    tokenId = result.tokenId;
                ...
                mapOfListings.put(tokenid,newListing);
                return #ok((true));
            }else{
              return #err((false));
            }
    ...
    public shared(msg) func delistNFT(tokenid:Nat):async Result.Result<(Bool),(Bool)>{
      switch (mapOfListings.get(tokenid)) {
        case (null) {return #err((false))};
        case (?result) {
          if(result.creator == msg.caller and result.isListed){
            let newListing : Listing = {
              tokenId = result.tokenId;
              price = 0;
              creator = result.creator;
    ...
    public query func getIsListed(tokenId: Nat) : async Bool {
   ...
   public query func getListingPrice(tokenId: Nat) : async Nat {
        switch (mapOfListings.get(tokenId)) {
            case (null) {return 0};
            case (?result) {
                if(result.isListed){
    ...
    system func postupgrade() {
        ...
        mapOfListings := HashMap.fromIter<ListID, Listing>(listingEntries.vals(), 1, Nat.equal, Hash.hash);
        usersEntries := [];
        tokensEntries := [];
        listingEntries := [];
    };
